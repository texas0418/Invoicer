...
export default function EditorScreen({ kind, existing, onDone }: Props) {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (existing) {
        ...
        setLines(existing.items.map((l) => ({ ...l })));
        setDueDays(
          String(
            Math.max(1, Math.round((existing.dueDateMs - existing.issueDateMs) / DAY_MS)),
          ),
        );
        setTaxPct((existing.taxRate * 100).toString());
        setDiscount(
          existing.discountCents > 0 ? (existing.discountCents / 100).toFixed(2) : '',
        );
        setPaymentLink(existing.paymentLink);
        setNotes(existing.notes);
        setCurrency(existing.currencySymbol);
      } else {
  ...
  const issueBaseMs = existing?.issueDateMs ?? Date.now();
  const dueDateMs =
    issueBaseMs + (parseInt(dueDays, 10) > 0 ? parseInt(dueDays, 10) : 14) * DAY_MS;
  ...
  const save = async () => {
    ...
    if (existing?.id != null) {
      ...
      if (existing.status === 'sent' && existing.dueDateMs !== dueDateMs) {
        await cancelReminder(notificationId);
        notificationId = await scheduleDueReminder({ ...existing, dueDateMs });
      }
    ...
    const inv: Invoice = {
      kind,
      number: kind === 'estimate' ? await nextEstimateNumber() : await nextInvoiceNumber(),
  ...
  return (
    ...
    >
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 64 }}>
        ...
        <Text style={s.label}>{kind === 'estimate' ? 'Valid for (days)' : 'Due in (days)'}</Text>
        <TextInput
          style={s.input}
          value={dueDays}
        ...
        />
        <Text style={s.hint}>
          {kind === 'estimate' ? 'Valid until' : 'Due date'}: {formatDate(dueDateMs)}
        </Text>
...
const s = StyleSheet.create({
  ...
  dialog: { backgroundColor: T.bg, borderRadius: 18, padding: 20 },
